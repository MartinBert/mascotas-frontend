...
// Views
import SalidasElements from './elements'
...
const SalidasForm = () => {
    ...
    useEffect(() => {
        const loadOutputData = async () => {
            ...
            // Recover output data for edit
            if (id !== 'nuevo') {
                const outputID = location.pathname.replace('/salidas/', '')
                const response = await api.salidas.findById(outputID)
                outputs_dispatch({ type: 'SET_OUTPUT', payload: response.data })
            }
        }
        loadOutputData()
    }, [auth_dispatch, outputs_dispatch, id])

    // ---------------- Loading state ---------------- //
    useEffect(() => {
        if (!auth_state.user) outputs_dispatch({ type: 'SET_LOADING', payload: true })
    ...
    useEffect(() => {
        const updateOutputsState = () => {
            ...
            outputs_dispatch({ type: 'SET_QUANTITY' })
        }
        updateOutputsState()
